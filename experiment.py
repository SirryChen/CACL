...
predata_file_path = "./predata/twibot22/"
model_save_path = f"./train_result/twibot22/"
# param_GAT = torch.load(model_save_path + f"pretrain_cd_model_HGT.pth")
param_HGT = torch.load(model_save_path + f"pretrain_cd_model_HGT_wrong_key.pth")
print("oo")

new_HGT = OrderedDict()
for param in param_HGT:
    new_key = param.replace('HGT', 'Hetero_Conv')
    new_HGT[new_key] = param_HGT[param]

torch.save(new_HGT, model_save_path + f"pretrain_cd_model_HGT.pth")
params = torch.load(model_save_path + f"pretrain_cd_model_HGT.pth")
print("oo")
