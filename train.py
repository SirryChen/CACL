...
def train(step, subgraph_loader):
    ...
    for graph_th, subgraph in enumerate(subgraph_loader.subgraph(tqdm_bar)):
        ...
        augmented_graph1, augmented_graph2 = adaptive_augment(copy.deepcopy(subgraph))
        ...
        total_loss.backward()
        optimizer_CL.step()
        subgraph_loader.cd_model.cd_encoder_decoder.update_conv_layer(CLModel.update_cd_model())
...
if __name__ == "__main__":
    ...
    device_cl = torch.device('cuda:6') if torch.cuda.is_available() else torch.device('cpu')

    predata_file_path = f"./predata/{dataset_name}/"
    model_save_path = f"./train_result/{dataset_name}/"
    experiment_result_save_path = f"./train_result/{dataset_name}/full_model_2/"
    ...
    num_neighbors = {edge_type: [50] * 3 if edge_type[0] != 'tweet' else [100] * 3 for edge_type in graph.edge_types}
    ...
    CDModel = ModCDModel(graph['user'].x.size(-1), args, cd_config, device_cd, graph.metadata())
    CDModel.cd_encoder_decoder.load_state_dict(torch.load(model_save_path + "pretrain_cd_model.pth"), strict=False)
    ...
    Encoder.init_first_layer(CDModel.cd_encoder_decoder.Hetero_Conv.state_dict())
