...
def get_users_feature():
    ...
    with open(dataset_file_path + 'node.json', 'r') as file:
        ...
        for user in tqdm(users, desc='加载用户特征'):
            each_user_feature = []
            if user.get("public_metrics") is None:
                continue
    ...
    for i, chunk_edges in enumerate(edges):
        for _, edge in tqdm(chunk_edges.iterrows(), desc=f'构建用户异质图-{i}'):
            ...
            if relation in ['follow', 'friend']:
                ...
                if users_feature_dict.get(source) is None or users_feature_dict.get(target) is None:
                    continue
            ...
            elif relation in ['post'] and tweets_text_dict.get(target) is not None:
                if edge_index_dict.get(('user', relation, 'tweet')) is None:
                    edge_index_dict['user', relation, 'tweet'] = []
                if users_feature_dict.get(source) is None or tweets_text_dict.get(target) is None:
                    continue
                if user_id_index_map.get(source) is None:
                    user_id_index_map[source] = max_user_index
                    max_user_index += 1
...
if __name__ == "__main__":
    dataset_file_path = "/data1/botdet/datasets/cresci-2015/"
    predata_file_path = "./predata/cresci-2015/"
